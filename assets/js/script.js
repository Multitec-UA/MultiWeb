/* script.js — multitecua.com. Vanilla, no jQuery (removed 2026-09-05 with Bootstrap 3).
 *
 * Three small jobs and one big one. The small ones: a hairline under the sticky header
 * once the page has scrolled, the phone menu, and closing that menu when a choice is
 * made. The FAQ needs nothing here any more — it is <details>/<summary>, which the
 * browser opens with a mouse, a keyboard or a screen reader on its own. The big one is
 * the events wheel, unchanged from 2026-08-31 and documented where it starts.
 *
 * Nothing here sets a width, a height or a position (tests/verify.sh group 15).
 */
(function () {
    'use strict';

    // ---- sticky header: a hairline once the page has moved -----------------------
    var head = document.querySelector('.site-head');
    if (head) {
        var stuck = false;
        var onScroll = function () {
            var now = window.scrollY > 8;
            if (now !== stuck) { stuck = now; head.classList.toggle('is-stuck', now); }
        };
    ...
    // ---- the phone menu ----------------------------------------------------------
    // aria-expanded is kept in step with the panel: a value that never changes tells a
    // screen reader the menu is shut while it is open (tests/verify.sh group 11).
    var toggle = document.getElementById('menu-toggle');
    var panel = document.getElementById('menu-item');
    if (toggle && panel) {
        var setMenu = function (open) {
            panel.classList.toggle('show-menu', open);
            panel.classList.toggle('hide-menu', !open);
            toggle.classList.toggle('c', open);
            toggle.setAttribute('aria-expanded', open ? 'true' : 'false');
        };
        toggle.addEventListener('click', function () {
            setMenu(toggle.getAttribute('aria-expanded') !== 'true');
        });
        // Any choice inside the panel closes it: every link in there is an in-page anchor
        // or another page, so leaving it open is never right.
